...
x <- data.frame(read.csv('/home/saur/constraint_alternative_test.txt', sep=",", col.names = z,header=FALSE))
...
print(plot)
ggsave(filename='/home/saur/constrained_alternative.jpg',plot)


y <- data.frame(read.csv('/home/saur/unconstraint_alternative_test.txt', sep=",", col.names = z,header=FALSE))
...
plot2 <- ggplot(data = yy,aes(x=variable, y= value, ymin=0.94)) + 
        ...
        theme(axis.text=element_text(size=18),axis.title=element_text(size=18),title = element_text(size = 24))
print(plot2)
ggsave(filename='/home/saur/unconstrained_alternative.jpg',plot2)